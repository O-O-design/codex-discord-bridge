...
function runProcess(command, args, { cwd, onStderr, onStdout, timeoutMs }) {
...
function buildDiscordPrompt({ author, authorProfile, channel, guild, content, recentContext, sandbox }) {
  return [
    ...
    DISCORD_LOCATION_NOTES.length > 0
...
export async function seedCodexSession(config, seedPrompt) {
  const outputDir = await mkdtemp(join(tmpdir(), "codex-discord-bridge-seed-"));
...
export async function askCodex(config, messageContext, options = {}) {
  ...
  const outputDir = await mkdtemp(join(tmpdir(), "codex-discord-bridge-codex-"));
  const outputFile = join(outputDir, "last-message.txt");
  ...
  try {
    await runProcess(config.codexCliPath, args, {
      cwd: process.cwd(),
      onStderr: (chunk) => options.onOutput?.({ source: "stderr", chunk }),
      onStdout: (chunk) => options.onOutput?.({ source: "stdout", chunk }),
      timeoutMs: config.codexTimeoutMs
    });
    return (await readFile(outputFile, "utf8")).trim();
  } finally {
